...
const ontimee = (e) => setproduct({...product,timee:e.target.value})
const ondatee = (e) => setproduct({...product,datee:e.target.value})
const onprcat = (e) => setproduct({...product,category:e.target.value})
const ontelee = (e) => setproduct({...product,telee:e.target.value})
// ---------------------------------upload image-------------------------------------------
  ...
  const uploadImage = (e) => {
    ...
    if(file){
      ...
      const addproduct=async(e)=>{
       ...
       setproduct({title:"",code:"",tele:"",key:"",imges:"",datee:""}) 
       setImagesitem([])
        }else{alert("أكمل باقي الحلول")
        }     
      }
     //   -------------------------------------------------------------------------------------------------- edit info --------------------      
...
<label htmlFor="htmlFormFileLg" className="form-label text-right text-primary">رفع السيرة الذاتية</label>
  ...
  <option selected>اختار المهنة</option>{
  category.map((item) => (
    <option value={item.name}>{item.name}</option>
            ))}
  </select>
  <div className="input-group input-group-lg border-success my-3 w-75 ms-auto required">
  <input type="text" className="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-sm"onChange={ontelee} value={product.tele} required />
...
<input type="date" className="form-control" aria-label="Sizing example input" min="2023-12-10" max="2023-12-25" aria-describedby="inputGroup-sizing-sm"onChange={ondatee} value={product.datee} required />
  <span className="input-group-text bg-primary text-light">التاريخ</span>
</div>
<div className="input-group  mb-3 w-75 ms-auto">
<input type="time" className="form-control" aria-label="Sizing example input"  min="09:00" max="17:00" aria-describedby="inputGroup-sizing-sm"onChange={ontimee} value={product.timee}  />
  <span className="input-group-text bg-primary text-light">الميعاد</span>
</div>
<input type="submit" className="btn btn-success my-3" value="إضافة متقدم"/>
</form>  
  </div>
